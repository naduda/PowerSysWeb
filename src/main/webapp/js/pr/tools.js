...
function Tools(){
	this.setPopupWindow = function (popup, parent){
	...
	function createAlarmColumns() {
		...
		ret.id = 'alarmColumns';
		ret.setAttribute('class', 'popupW autoClose');
		...
		$('input[name="alarmColumns"]').change(function(e){
			...
			if(model) model.alarm.showHideColumn(ind, $(this)[0].checked);
		});
		return ret;
	};

	this.timestamp2date = function(ts){
		var d = new Date(ts),
	...
	this.dateToolbar = function(parent, onDateChange){
		var toolbar = {
			lFrom: 	elt('span',{id:'${keyPeriodFrom}'}),
			from: 	elt('input',{class:'datePicker menubutton',
														style:'margin:0 5px;'}),
			lTo: 		elt('span',{id:'${keyTo}'}),
			to: 		elt('input',{class:'datePicker menubutton',
														style:'margin:0 5px;'}),
			div: 		parent
		}
